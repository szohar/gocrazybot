...
unsigned int Shaft_Speed 	= 0x00;		//-- Stores actual speed
unsigned int N;
int encoder_count = 0x00;
int time_in_sec;
int i, count, random;
int Tick_count = 0x00;
...
void Init_Timer0(unsigned char reload0);
...
void Init_Port(void) //-- Configures the Crossbar & GPIO ports
{
		...
		P1MDIN |= 0x08;		//-- Pin P0.3 is for input of Tacho Signal so write a '1' to it
...
void Init_Timer0(unsigned char reload0)
{
	...
	TH0 = reload0;
	TL0 = TH0;
	TCON  = 0x14; 		//-- Enable Timer0 Interrupt	
	IE    = 0x06;		//-- Start Timer0
}
...
{
	TF0 = 1;
	if(encoder_count > 0)				//-- Leave Actual Speed Display in 
										//-- Current State if Tacho has not 
										//-- Counted (Removes divide by 0 case)
	{
		Shaft_Speed = 310/encoder_count;	//-- Changed from 3000000 as proportionality
											//-- constant K is 9672 times smaller due to
											//-- 312 pulses from encoder per shaft
											//-- revolution and 1:31 gearing
		...
		{
			actual_speed = Shaft_Speed;		// Display Actual Speed
		}
	}
	
	encoder_count = 0;				// Reset Tacho Counter
}
...
void main(void)
{
		...
		while(1) //-- go on forever
		{
			...
			printf("Speed(RPM):%5u", actual_speed);
