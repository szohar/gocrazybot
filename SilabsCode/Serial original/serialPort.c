...
void UART0_ISR(void) interrupt 4
{
	...
	//	 port5 = P5;
	    ...
	    if( (received_byte>9)&&(received_byte<16) )
			    {
				        lcd_clear();
	...
	/*		
  //test that all is sent properly:
	command = 12;
	port5 = 13;
	message = 14;
    received_byte = 15;
...
void main(void)
{
		...
		while(1) //-- go on forever
		{
           ...
           if (new_cmd_received == 1)
	       {
					...
					if( (P5 != 0x0E)&&(P5 != 0x0D)&&(P5 != 0x0B)&&(P5 != 0x07))
					  {
		                    ...
		                    if (received_byte <128 )
							{
								...
								//-- Display
								    ...
								    printf(" Going Left!!!  " );
									lcd_goto(0x40);   //-- go to Second Row
									printf("   received: %d", received_byte);
									huge_delay(20);
								//	lcd_clear();
					        }
							else
							{
		                     ...
		                     if(received_byte == 132)
								...
								//-- Display
									lcd_goto(0x00) ;   //-- go to first Row
								   // printf("    Stop!!  ");
								   printf("     pFlag : %d", prevTransmitFlag );
									lcd_goto(0x40);   //-- go to Second Row
					     	
									printf("     Flag : %d", transmitFlag ); 
							...
							if( (received_byte > 3)&&(received_byte != 132))
							{
								lcd_clear();
									//-- Display
								lcd_goto(0x00) ;   //-- go to first Row
